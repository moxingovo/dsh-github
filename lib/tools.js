...
export function parseSearchArgs(args, maxPerPage) {
    ...
    if (perPage > maxPerPage)
        throw new Error('perPage exceeds the maximum of ' + maxPerPage);
    return { query: args.query.trim(), kind: kind, page, perPage };
}
/**
...
export function parseGetArgs(args) {
    ...
    if (!GET_KINDS.includes(kind))
        throw new Error('kind must be one of repo, issue, file');
    ...
    if (!NAME_PATTERN.test(owner))
        throw new Error('owner must contain only letters, digits, dots, dashes, and underscores');
    if (!NAME_PATTERN.test(repo))
        throw new Error('repo must contain only letters, digits, dots, dashes, and underscores');
    if (kind === 'issue') {
        if (args.number === undefined || !Number.isInteger(args.number) || args.number < 1)
            throw new Error('number must be a positive integer for kind issue');
        return { kind, owner, repo, number: args.number };
    }
    if (kind === 'file') {
        const path = args.path === undefined ? '' : args.path.trim();
        if (path.length === 0)
            throw new Error('path must be a non-empty string for kind file');
 ...
 */
export function githubFileOutput(file, maxChars) {
...
export function formatSearchOutput(result) {
    ...
    const lines = result.items.map((item) => {
        ...
        if (item.author.length > 0)
            meta.push('by ' + item.author);
        if (item.stars > 0)
            meta.push('stars: ' + item.stars);
        if (item.comments > 0)
            meta.push('comments: ' + item.comments);
        if (item.state.length > 0)
            meta.push(item.state);
        if (item.createdAt.length > 0)
            meta.push('created: ' + item.createdAt.slice(0, 10));
...
export function formatRepoOutput(detail) {
    const lines = [
...
export function formatIssueOutput(detail) {
...
export function formatFileOutput(output) {
 ...
 */
export function applyGithubSearchTool(ctx, service, maxPerPage, timeoutMs) {
    ctx.tools.register(defineTool({
        ...
        presentResult: (_args, _result) => undefined,
    }));
}
/**
 * Register the github_get tool.
 */
export function applyGithubGetTool(ctx, service, maxChars, timeoutMs) {
    ctx.tools.register(defineTool({
        name: 'github_get',
        ...
        async execute(args, exec) {
            ...
            if (input.kind === 'issue') {
                const issue = await service.issue({ owner: input.owner, repo: input.repo, number: input.number }, exec.signal);
                return { kind: input.kind, issue: { ...issue, labels: [...issue.labels] } };
            }
            const file = await service.file({ owner: input.owner, repo: input.repo, path: input.path, ...(input.ref === undefined ? {} : { ref: input.ref }) }, exec.signal);
            return { kind: input.kind, file: githubFileOutput(file, maxChars) };
        },
        presentCall: (args) => ({ card: 'generic', title: args.kind + ' ' + args.owner + '/' + args.repo, kind: 'search', rawInput: args.kind + ' ' + args.owner + '/' + args.repo }),
        presentResult: (_args, _result) => undefined,
    }));
}
/** Render the discriminated github_get output value. */
function renderGetOutput(value) {
    switch (value.kind) {
        case 'repo': return formatRepoOutput(value.repo);
        case 'issue': return formatIssueOutput(value.issue);
        default: return formatFileOutput(value.file);
    }
}
